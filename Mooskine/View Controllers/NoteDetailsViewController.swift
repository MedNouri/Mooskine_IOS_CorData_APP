...
class NoteDetailsViewController: UIViewController {
 ...
 var dataControler:DataController!
    ...
    override func viewDidLoad() {
        ...
        if let creationDate = note.creadtionDate {
                 navigationItem.title = dateFormatter.string(from: creationDate)
             }
             textView.attributedText = note.attrtext
...
extension NoteDetailsViewController: UITextViewDelegate {
    func textViewDidEndEditing(_ textView: UITextView) {
        note.attrtext  = NSAttributedString(string: textView.text)
...
extension NoteDetailsViewController {
    ...
    func makeToolbarItems() -> [UIBarButtonItem] {
        ...
        let space = UIBarButtonItem(barButtonSystemItem: .flexibleSpace, target: nil, action: #selector(spaceActionTapped))
        let bold = UIBarButtonItem(image: #imageLiteral(resourceName: "toolbar-bold"), style: .plain, target: self, action: #selector(boldActionTapped))
           let red = UIBarButtonItem(image: #imageLiteral(resourceName: "toolbar-underline"), style: .plain, target: self, action: #selector(redActionTapped))
   let cow = UIBarButtonItem(image: #imageLiteral(resourceName: "toolbar-cow"), style: .plain, target: self, action: #selector(cowActionTapped))
        
        return [trash, space,bold,red,cow]
    }

    
    @objc func boldActionTapped(){
        let newText = textView.attributedText.mutableCopy() as! NSMutableAttributedString
            ...
            note.attrtext = textView.attributedText
        try? dataControler?.viewContext.save()
    }
    
    
    @objc func spaceActionTapped(){
        
    }
    
    
    @objc func redActionTapped(){
        let newText = textView.attributedText.mutableCopy() as! NSMutableAttributedString
    ...
    @objc func cowActionTapped(){
        
        let backgroundContext:NSManagedObjectContext! = dataControler.backgroundContect
        let newText = textView.attributedText.mutableCopy() as! NSMutableAttributedString
        ...
        backgroundContext.perform {
            ...
            let cowText = Pathifier.makeMutableAttributedString(for: selectedText, withFont: UIFont(name: "OpenSans-Bold", size: 56)!, withPatternImage: #imageLiteral(resourceName: "texture-cow"))
               ...
               backgroundNote.attrtext = newText
    ...
    func addNotificationObserver(){
        removeNotification()
        savedObeser = NotificationCenter.default.addObserver(forName: .NSManagedObjectContextObjectsDidChange, object: dataControler.viewContext, queue: nil, using: handelNotification)
    }
    
    func removeNotification(){
        
    }
    func handelNotification(notification:Notification){
        
        DispatchQueue.main.async {
            self.textView.attributedText = self.note.attrtext
        }
